...
cea-hf-llama.py: Perform the CEA task using the Llama Model API from HuggingFace, 
    while being independently run on runpod.io 
...
def write_csv(wfilename, wrow, wcol, wurl):
    ...
    output_filename = "output/cea-llama3-8b-instruct.csv"
...
def clean_data(raw_csv_data):
    ...
    session = model.chat.completions.create(
        model = MODEL_NAME,
        messages = [{"role": "user", "content": clean_data_msg}],
        temperature = 0.25,
        max_tokens = 4096,
        top_p=0.95,
    )
    csv_data_response = session.choices[0].message.content.strip()
...
def process_cell(filename, row_index, col_index, cell_data, csv_data, entity_dict):                  
    ...
    ## LLM performs matching
        ...
        session = model.chat.completions.create(
            model = MODEL_NAME,
            messages = [{"role": "user", "content": matching_msg}],
            temperature = 0.25,
            max_tokens = 4096,
            top_p=0.95,
        )
        response = session.choices[0].message.content.strip()        
...
if __name__ == '__main__':
    ...
    event.clear()
    input_folderpath = "data/r1/tables"
